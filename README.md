...
A Rust web application using Leptos for server-side rendering, deployed to Cloudflare Workers. Demonstrates best practices from [AGENTS.md](https://github.com/lemmih/best-practice/blob/main/AGENTS.md).
...
- **Nix**: Uses `flake.nix` with pinned nixpkgs (`nixos-25.11`) for reproducible builds
...
- **wasm-bindgen**: Pins exact version (`0.2.106`) to match CLI and library
...
- **leptosfmt**: Leptos view! macro formatting
